...
const PILLARS = [
  { key: 'spiritual', label: 'Spiritual', icon: '✝️', placeholder: "e.g. 'First prayer unprompted'", about: "As your child comes to study their place in this world, a question will surface that no other pillar can answer: What is real beyond what I can measure, and how do I live in light of it? This pillar exists because the spiritual dimension is not optional — it is the environment in which everything else operates. The goal is not a child who performs religion. The goal is a person who can recognize what is genuinely good and pursue it, recognize what wears the mask of good and test it, recognize what attacks them spiritually and name it, stand firm when structures are compromised — even structures that claim holiness — and transmit what they've learned without coercion. The primary tool is the Fruit Test (Matthew 7:15–20): not by words, office, or credentials, but by what a person or institution produces over time." },
  ...
  { key: 'education', label: 'Education & Career', icon: '🎓', placeholder: "e.g. 'Completed first job shadow'", about: "A career is not an identity you absorb from a comment — it is a hypothesis you test with evidence. The most dangerous pattern in career formation is the one-comment trap: a single remark from a teacher, relative, or stranger ('you'd make a great lawyer') lodges in a child's identity and silently steers years of decisions. The compounding cost of correction grows every year — loans, credentials, networks, family, location all pile up against deviation. This pillar teaches: expose without directing (6–12), test without absorbing (13–18), choose with evidence and preserve pivot capability (18–25), master and maintain adaptability (25–35). All paths are valid — college, trade, military, entrepreneurship. The metric is fit, not prestige. Every career suggestion must pass through a filter: who said it, what evidence supports it, have I tested it, and what would I lose if it's wrong?" },
  ...
  { key: 'dimensional_navigation', label: 'Life Navigation', icon: '🧭', placeholder: "e.g. 'Identified a hidden consequence'", about: "Every decision you make has consequences you can see and consequences you cannot see — yet. This pillar teaches your child to perceive what is hidden: the delayed consequence that arrives in a year, the second-order effect that hits someone else, the pattern that compounds across a decade, the force acting on their trajectory that they haven't named yet. It starts simple: 'If you do this today, what happens next week?' It broadens: what are the forces acting on your life right now — and which ones are you choosing vs. which ones are just carrying you? The goal is not prediction. The goal is perception. A child who can see one move further than their peers — who can ask 'what happens after what happens next?' — makes fundamentally better decisions. Not because they're smarter, but because they're looking where others aren't." },
  { key: 'civic', label: 'Civic & Institutional', icon: '🏛️', placeholder: "e.g. 'Named all 12 layers of governance'", about: "The people don't know their true power. This pillar exists to make sure your child does. The daily noise of politics — the outrage cycle, the partisan theater, the personality worship — is designed to keep citizens looking UP at leaders instead of looking DOWN at the plank those leaders stand on. The first lesson is the image: a crowd unknowingly supports the very platform a politician speaks from. If the crowd walks away, the platform collapses. Your child will learn: I am not a spectator of government. I am the infrastructure government depends on. My attention, money, labor, vote, and participation are the plank. The goal is not political identity. The goal is structural literacy — mapping every layer from HOA to federal, knowing who holds every seat, when every election happens, how money flows, where the real decisions are made (usually not where the cameras are), and what happens when citizens actually show up. By 18, the child should be able to walk into any election knowing exactly who is on their ballot, what each seat controls, who funds those candidates, and why most of their neighbors cannot say the same." },
    ...
    if (activeProgram) {
      ...
      return (
        <div style={s.container}>
          ...
          {prog.intro && (
            <details style={{ marginBottom: 16, background: '#f8fafe', padding: 12, borderRadius: 8, border: '1px solid #e4edfa' }}>
              <summary style={{ cursor: 'pointer', fontWeight: 600, fontSize: 13, color: '#4a6fa5' }}>📖 About This Program</summary>
